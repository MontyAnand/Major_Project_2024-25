...
class Proxy {
    private: 
        ...
        static bool searchFolder(const std::string directory, const std::string folderName) {
            try {
        ...
        static bool searchFile(const std::string directory, const std::string fileName){
            try{
                for (const auto& entry : fs::directory_iterator(directory)){
        ...
        static void modifyCN(std::string &CN){
            for(char &c : CN){
                if(c == '.'){
                    c = '_';
                }
        ...
        static bool writeCSRConfFile(std::string modifiedCN){
        ...
        static bool prepareCertificate(std::string CN){
            modifyCN(CN);
            try{
        ...
        static int sslContextCallback(SSL *ssl, void *arg){
        ...
        void handleNewConnection(int epoll_fd, int listener){
            ...
            SSL_set_fd(server_ssl, server_socket);
